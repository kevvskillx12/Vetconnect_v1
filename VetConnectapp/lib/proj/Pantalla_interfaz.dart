...
import 'Informacion_mascota.dart'; // Importa la pantalla de información de mascotas
import 'Idioma.dart';
...
class _VetConnectScreenState extends State<VetConnectScreen> {
  ...
  String correo = "";
  String contrasena = "";
  int usuarioId = 0; //usamos un valor predeterminado (por ejemplo 0)

  @override
  void initState() {
    super.initState();
    _loadUserData(); // Cargar correo, contraseña y usuarioId
    _loadCitas();
  ...
  Future<void> _loadUserData() async {
    ...
    setState(() {
      ...
      usuarioId = prefs.getInt('usuarioId') ?? 0; // Si no existe, asigna 0
    });
    print(
        "UsuarioId: $usuarioId"); //verifica que el usuarioId se lea correctamente
  }
  ...
  void _loadCitas() async {
    ...
    setState(() {
      upcomingAppointments = citas;
      hasUpcomingAppointments = citas.isNotEmpty;
    });
  }
  ...
  void _agendarCita(String cita) {
    setState(() {
      upcomingAppointments.add(cita);
      ...
      appBar: AppBar(
        ...
        title: Row(
          children: [
            ...
            // Icono del perro que navega a Informacion_mascota.dart
            IconButton(
  ...
  void _showMenu(BuildContext context) {
    ...
    showMenu(
      ...
      items: [
        ...
        PopupMenuItem(
          value: 'servicios_veterinarios',
          child: const ListTile(
            leading: Icon(FontAwesomeIcons.paw, size: 20),
            title: Text('Servicios Veterinarios'),
          ),
        ),
        PopupMenuItem(
          value: 'historial_citas',
          child: const ListTile(
      ...
      // Aquí puedes agregar más condiciones para otros valores de 'value'
    });
  }
}
